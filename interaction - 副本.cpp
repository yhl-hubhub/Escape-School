...
Monster_TypeDef monster1;
monster1.start_x=275;
monster1.start_y=250;
...
int mstMove(Monster_TypeDef* monster)
{
	PIMAGE pUpd = newimage();
    //showScore();
    //delay_ms(120);
    ...
    while(flag)
	{
		...
		{
            monster->mstx-=10;
            if(isTouchWall(LEFT,monster->mstx,monster->msty))
				mstx+=10;
	        getimage(pUpd,"img\\guaiwu1.jpg");
	        putimage(monster->mstx,monster->msty,pUpd);/**/
	        flag=0;
        }
        //右
        else if(inum==RIGHT)
		{
            monster->mstx+=10;
            if(isTouchWall(RIGHT,monster->mstx,monster->msty))
				mstx-=10;
	        getimage(pUpd,"img\\guaiwu1.jpg");
	        putimage(monster->mstx,monster->msty,pUpd);/**/
            flag=0;
        ...
        {
            monster->msty-=10;
            if(isTouchWall(UP,monster->mstx,monster->msty))
				msty+=10;
	        getimage(pUpd,"img\\guaiwu1.jpg");
	        putimage(monster->mstx,monster->msty,pUpd);/**/
            flag=0;
        ...
        {
            monster->msty+=10;
            if(isTouchWall(DOWN,monster->mstx,monster->msty))
				msty-=10;
	        getimage(pUpd,"img\\guaiwu1.jpg");
	        putimage(monster->mstx,monster->msty,pUpd);/**/
            flag=0;
